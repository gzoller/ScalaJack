...
case class PlainClassTypeAdapter[T](
  ...
  override def dbKeys: List[ClassFieldMember[T, Any]] =
    (fieldMembersByName.values.toList ++ nonConstructorFields.values.toList)
  ...
  def read[WIRE](path: Path, reader: Reader[WIRE], isMapKey: Boolean): T = {
    ...
    val concreteTypes = typeMembersByName.map {
      case (name, tm) =>
        (tm.typeSignature.toString, tm.copy(runtimeConcreteType = {
          ...
          reader
            .scanForType(path, name, reader.jackFlavor.typeValueModifier)
            .orElse {
              ...
              throw new ReadMissingError(
                reader.showError(
                  path,
                  s"Class $className missing type hint for type member ${tm.typeSignature.toString} (looking for $name)"
                )
              )
            }
        }))
    ...
    val both = ClassHelper.applyConcreteTypeMembersToFields(
      concreteTypes,
      typeMembersByName,
      fieldMembersByName ++ nonConstructorFields
    )
      ...
      case objectFieldResult: ObjectFieldsRead =>
        val finalConstructorArgList =
          if (!objectFieldResult.allThere) {
            ...
            val justConstructorArgs =
              objectFieldResult.objectArgs.take(fieldMembersByName.size)
            val justConstructorIsSet =
              objectFieldResult.fieldSet.take(fieldMembersByName.size)

            justConstructorArgs.zip(justConstructorIsSet).zipWithIndex.map {
              // Missing with default value
              case ((_, false), index)
                  if fieldArray(index).defaultValue.isDefined =>
              ...
              case ((_, false), index) =>
                ...
                throw new ReadMissingError(
                  reader.showError(
                    path,
                    s"Class $className missing field ${fieldArray(index).name}"
                  )
                )
        ...
        val asBuilt = constructorMirror
          .apply(finalConstructorArgList.toIndexedSeq: _*)
          .asInstanceOf[T]

        // Populate non-constructor fields (getters/setters, vars)
        val fieldsAndArgs =
          if (!objectFieldResult.allThere) {
            val nonConstructorArgs =
              objectFieldResult.objectArgs.takeRight(nonConstructorFields.size)
            val nonConstructorIsSet =
              objectFieldResult.fieldSet.takeRight(nonConstructorFields.size)
            ...
            nonConstructorFields.values
              ...
              .collect {
                // Missing but optional
                case (field, (_, false))
                    if field.isOptional && !field.hasOptionalAnnotation =>
                  (field, None)
                ...
                case (field, (_, false)) if !field.hasOptionalAnnotation =>
                  ...
                  throw new ReadMissingError(
                    reader.showError(
                      path,
                      s"Class $className missing field ${field.name}"
                    )
                  )
                // Anything else... as-read
                case (field, (arg, true)) =>
            ...
            nonConstructorFields.values
              .zip(
                objectFieldResult.objectArgs
                  .takeRight(nonConstructorFields.size)
              )
              .toList
  ...
  def write[WIRE](t: T,
          ...
          (fieldMembersByName ++ nonConstructorFields).values.collectFirst {
            ...
            case Some(tmWithActualType) =>
              val typeMemberValue = writer.jackFlavor.typeValueModifier match {
                case Some(fn) =>
                  fn.unapply(tmWithActualType.runtimeConcreteType.get)
              ...
              extras.append(
                (
                  ...
                  ExtraFieldValue(
                    typeMemberValue,
                    writer.jackFlavor.stringTypeAdapter
                  )
                )
              )
    ...
    writer.writeObject(
      t,
      ClassHelper.applyConcreteTypeMembersToFields(
        typeMembersWithRealTypes,
        typeMembersByName,
        fieldMembersByName ++ nonConstructorFields
      ),
      out,
