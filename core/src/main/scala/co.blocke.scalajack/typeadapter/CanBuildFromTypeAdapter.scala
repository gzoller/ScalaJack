    ...
    val matchingTypeAdapters = implicitConversions flatMap { method =>
      ...
      if (tt.tpe <:< typeOf[GenMapLike[_, _, _]] && elementTypeAfterSubstitution <:< typeOf[(_, _)]) {
        ...
        Some(CanBuildFromTypeAdapter[E, T](
          ...
          */
        Some(CanBuildFromTypeAdapter[E, GenTraversableOnce[E]](canBuildFrom.asInstanceOf[CanBuildFrom[Any, E, GenTraversableOnce[E]]], elementTypeAdapter.asInstanceOf[TypeAdapter[E]]))
      }
    }
...
case class CanBuildFromTypeAdapter[Elem, To <: GenTraversableOnce[Elem]](canBuildFrom: CanBuildFrom[_, Elem, To], elementTypeAdapter: TypeAdapter[Elem]) extends ArrayTypeAdapter[To, Elem] {
