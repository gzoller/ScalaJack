...
object CaseClassTypeAdapterFactory extends TypeAdapterFactory.FromClassSymbol {
  ...
  override def typeAdapterOf[T](
  ...
  )(implicit context: Context, tt: TypeTag[T]): TypeAdapter[T] =
    if (classSymbol.isCaseClass) {
      ...
      val fieldMembers =
        for (((member, param2), index) <- (params1 zip params2).zipWithIndex) yield {
          ...
          val memberTypeAdapter =
            context.typeAdapter(memberType).asInstanceOf[TypeAdapter[Any]]
      ...
      val justDeclaredFieldTypes =
        orderdFieldMembers.map(_._2.declaredValueType).toList
