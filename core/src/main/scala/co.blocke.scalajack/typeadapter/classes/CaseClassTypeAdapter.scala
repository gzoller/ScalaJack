...
case class CaseClassTypeAdapter[T](
    ...
    isSJCapture:        Boolean,
    collectionName:     Option[String])(implicit context: Context, tt: TypeTag[T])
  ...
  def read[WIRE](path: Path, reader: Reader[WIRE], isMapKey: Boolean): T = {
    val concreteTypes = typeMembersByName.map {
      case (name, tm) =>
        (
          tm.typeSignature.toString,
          tm.copy(
            runtimeConcreteType = // 1. Look Ahead for type hint
    ...
    reader
      .readObjectFields[T](path, isSJCapture, ClassHelper.applyConcreteTypeMembersToFields(concreteTypes, typeMembersByName, fieldMembersByName)) match {
  ...
  def write[WIRE](t: T, writer: Writer[WIRE], out: Builder[WIRE, WIRE], isMapKey: Boolean): Unit = {
    ...
    writer.writeObject(
      t,
      ClassHelper.applyConcreteTypeMembersToFields(typeMembersWithRealTypes, typeMembersByName, fieldMembersByName),
      out,
      extras.toList)
  }
