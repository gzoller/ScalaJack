...
import co.blocke.scalajack.typeadapter.{ CanBuildFromTypeAdapter, CanBuildMapTypeAdapter, CollectionDeserializer, OptionTypeAdapter }
...
object JsonCanBuildFromTypeAdapter extends TypeAdapterFactory.<:<.withOneTypeParam[GenTraversableOnce] {

  override def create[E, C <: GenTraversableOnce[E]](next: TypeAdapterFactory)(implicit context: Context, tt: TypeTag[C], ttX: TypeTag[GenTraversableOnce[E]], ttElement: TypeTag[E]): TypeAdapter[C] = {
    ...
    val matchingTypeAdapters = implicitConversions flatMap { method =>
      ...
      val elementTypeAdapter = context.typeAdapter(elementTypeAfterSubstitution).asInstanceOf[TypeAdapter[E]]

      val companionInstance = currentMirror.reflectModule(companionSymbol).instance
      val canBuildFrom = currentMirror.reflect(companionInstance).reflectMethod(method).apply().asInstanceOf[CanBuildFrom[Any, E, C]]
      ...
      if (tt.tpe <:< typeOf[GenMapLike[_, _, _]] && elementTypeAfterSubstitution <:< typeOf[(_, _)]) {
        ...
        Some(CanBuildFromTypeAdapter(
          ...
          null,
          canBuildFrom.asInstanceOf[CanBuildFrom[Any, Any, GenTraversableOnce[Any]]],
          elementTypeAdapter.asInstanceOf[TypeAdapter[Any]]))
      }
    }

    matchingTypeAdapters.headOption.map(_.asInstanceOf[TypeAdapter[C]]).getOrElse(next.typeAdapterOf[C])
  }
