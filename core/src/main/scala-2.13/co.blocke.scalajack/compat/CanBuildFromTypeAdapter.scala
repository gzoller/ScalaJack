...
case class CanBuildFromTypeAdapterFactory(jackFlavor: JackFlavor[_], enumsAsInt: Boolean) extends TypeAdapterFactory {
  ...
  private def buildListTA[Elem, To >: Null <: IterableOnce[Elem]](companionInstance: Any, method: MethodSymbol, elemTypeAdapter: TypeAdapter[Elem]) = {
    val builderFactory = reflect(companionInstance).reflectMethod(method)
...
case class CanBuildMapTypeAdapter[Key, Value, To <: Map[Key, Value]]( //) >: Null <: GenMapLike[Key, Value, To]](
  ...
  def read[WIRE](path: Path, reader: Reader[WIRE], isMapKey: Boolean): To =
