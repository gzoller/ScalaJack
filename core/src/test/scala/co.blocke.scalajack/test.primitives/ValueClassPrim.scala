...
class ValueClass() extends FunSpec with Matchers {
  ...
  describe("---------------------------------\n:  ValueClass Primitives Tests  :\n---------------------------------") {
    describe("+++ Positive Tests +++") {
      ...
      it("Value class of UUID") {
        ...
        val js = sj.render(inst)
        assertResult("""{"vc":"54cab778-7b9e-4b07-9d37-87b97a011e55"}""") { js }
        assertResult(inst) {
          sj.read[VCUUID](js)
        }
      }
