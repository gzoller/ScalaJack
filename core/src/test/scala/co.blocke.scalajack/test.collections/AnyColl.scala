...
class AnyColl() extends FunSpec with Matchers {
  ...
  describe("--------------------------\n:  Any Collection Tests  :\n--------------------------") {
    ...
    it("List works (Class)") {
      ...
      val js = sj.render(inst)
      assertResult("""[{"_hint":"co.blocke.scalajack.test.collections.Player","name":"Mike","age":34},{"_hint":"co.blocke.scalajack.test.collections.Player","name":"Sarah","age":29}]""") { js }
    ...
    it("Map works (Class,Int)") {
      ...
      val js = sj.render(inst)
      assertResult("""{"{\"_hint\":\"co.blocke.scalajack.test.collections.Player\",\"name\":\"Mike\",\"age\":34}":1,"{\"_hint\":\"co.blocke.scalajack.test.collections.Player\",\"name\":\"Sarah\",\"age\":29}":2}""") { js }
