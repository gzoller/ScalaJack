      ...
      describe("+++ Positive Tests +++") {
        ...
        it("BigInt must work") {
          ...
          val expected = JObject() ~ ("bi1" -> JInt(
            BigInt("-90182736451928374653345")
          )) ~ ("bi2" -> JInt(BigInt("90182736451928374653345"))) ~ ("bi3" -> JInt(
              0
            )) ~ ("bi4" -> JNull)
          assertResult(Diff(JNothing, JNothing, JNothing)) { js4s.diff(expected) }
          assertResult(inst) {
        ...
        it("Enumeration must work (not nullable)") {
          ...
          val js4s = sj.render(inst)
          val expected = JObject() ~ ("e1" -> JString("Small")) ~ ("e2" -> JString(
            "Medium"
          )) ~ ("e3" -> JString("Large")) ~ ("e4" -> JNull) ~ ("e5" -> JString(
              "Medium"
            ))
          assertResult(Diff(JNothing, JNothing, JNothing)) { js4s.diff(expected) }
