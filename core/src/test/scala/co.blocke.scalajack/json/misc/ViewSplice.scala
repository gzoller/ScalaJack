...
class ViewSplice() extends AnyFunSpec with Matchers {
  ...
  describe("-----------------------\n:  View/Splice Tests  :\n-----------------------") {
    ...
    it("Must enforce required constructor fields") {
      val master = Master("Greg", List("a", "b"), List(Encapsulated("x", false), Encapsulated("y", true)), Encapsulated("Nest!", true), Some("wow"), Map("hey" -> 17, "you" -> 21), true, 99123986123L, Num.C, 46)
      val msg = """View master object co.blocke.scalajack.json.misc.Master is missing field bar required to build view object Encapsulated"""
      the[ViewException] thrownBy sj.view[Encapsulated](master) should have message msg
    }
    it("Must spliceInto") {
      val master = Master("Greg", List("a", "b"), List(Encapsulated("x", false), Encapsulated("y", true)), Encapsulated("Nest!", true), Some("wow"), Map("hey" -> 17, "you" -> 21), true, 99123986123L, Num.C, 46)
