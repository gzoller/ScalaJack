...
class TypeMembers extends AnyFunSpec with GivenWhenThen with BeforeAndAfterAll {
  ...
  describe(
    "-----------------------------\n:  Externalized Type Tests  :\n-----------------------------"
      ...
      it("Read and match") {
        val json =
          """{"Giraffe":"co.blocke.scalajack.json.misc.FancyBody","id":"ABC","body":{"message":"Hello"}}"""
      ...
      it("Write -- Concrete T value") {
        ...
        val expected =
          """{"Giraffe":"co.blocke.scalajack.json.misc.FancyBody","id":"DEF","body":{"message":"BOO"}}"""
      ...
      it("Write -- Trait T value") {
        ...
        val expected =
          """{"Giraffe":"co.blocke.scalajack.json.misc.Body","id":"DEF","body":{"_hint":"co.blocke.scalajack.json.misc.FancyBody","message":"BOO"}}"""
        assertResult(expected) {
      ...
      it("Wrapped") {
        ...
        assertResult(
          """{"foo":25,"env":{"Giraffe":"co.blocke.scalajack.json.misc.FancyBody","id":"abc","body":{"message":"msg here"}}}"""
        ) { js }
        assertResult(inst) {
          sj.read[Bigger](js)
        }
      ...
      it("Type modifier works") {
        ...
        assertResult(
          """{"Giraffe":"Body","id":"DEF","body":{"_hint":"co.blocke.scalajack.json.misc.FancyBody","message":"BOO"}}"""
        ) {
            js
          }
        assertResult(value) {
      ...
      it("Handles mutliple externalized types (bonus: with modifier)") {
        ...
        assertResult(
          """{"Hippo":"Hobby","Giraffe":"Body","id":"DEF","body":{"_hint":"co.blocke.scalajack.json.misc.FancyBody","message":"BOO"},"hobby":{"_hint":"co.blocke.scalajack.json.misc.InsideHobby","desc":"stamps"}}"""
        ) {
            js
          }
        assertResult(value) {
      ...
      it("Works with ParseOrElse") {
        val js =
          """{"Giraffe":"co.blocke.scalajack.json.misc.FancyBody","id":"DEF","body":{"bogus":"BOO"}}"""
