...
class ComplexRelationships extends FunSpec with GivenWhenThen with BeforeAndAfterAll {
  ...
  describe("---------------------------------\n:  Complex Relationships Tests  :\n---------------------------------") {
    it("Child class declares a type parameter not provided by the parent (trait)") {
      ...
      assertResult("""{"_hint":"co.blocke.scalajack.json.misc.Child","a":1,"b":true,"c":"here"}""") { js }
      assertResult(inst) {
    ...
    it("Parameterized type implements non-parameterized trait") {
      ...
      val js = sj.render(inst)
      assertResult("""{"_hint":"co.blocke.scalajack.json.misc.Dog","name":"Fido","kind":{"_hint":"co.blocke.scalajack.json.misc.Dog","name":"Larry","kind":true}}""") { js }
    ...
    it("Parameterized type implements non-parameterized trait (using Long for parameter type") {
      ...
      val js = sj.render(inst)
      assertResult("""{"_hint":"co.blocke.scalajack.json.misc.Dog","name":"Fido","kind":{"_hint":"co.blocke.scalajack.json.misc.Dog","name":"Larry","kind":15}}""") { js }
    ...
    it("Parameterized type implements non-parameterized trait (using enum for parameter type)") {
      ...
      val js = sj.render(inst)
      assertResult("""{"_hint":"co.blocke.scalajack.json.misc.Dog","name":"Fido","kind":"Pug"}""") { js }
    ...
    it("Parameterized type implements non-parameterized trait (with Map)") {
      ...
      val js = sj.render(inst)
      assertResult("""{"_hint":"co.blocke.scalajack.json.misc.Dog","name":"Fido","kind":{"Larry":true}}""") { js }
