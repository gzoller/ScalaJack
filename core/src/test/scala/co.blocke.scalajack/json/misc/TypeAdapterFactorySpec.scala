...
object Factory7 extends TypeAdapterFactory.=:=.withTwoTypeParams[ListMap] {
  def create[E1, E2](next: TypeAdapterFactory)(implicit tt: TypeTag[ListMap[E1, E2]], ttElement1: TypeTag[E1], ttElement2: TypeTag[E2]): TypeAdapter[ListMap[E1, E2]] =
...
object Factory6 extends TypeAdapterFactory.=:=.withOneTypeParam[Set] {
  def create[E](next: TypeAdapterFactory)(implicit context: Context, tt: TypeTag[Set[E]], ttElement: TypeTag[E]): TypeAdapter[Set[E]] =
    (new TA6[Set[E]]()).asInstanceOf[TypeAdapter[Set[E]]]
}

trait EmptyTypeAdapter[T] extends TypeAdapter[T] {
  def read[WIRE](path: Path, reader: Reader[WIRE], isMapKey: Boolean): T = null.asInstanceOf[T]
...
class TypeAdapterFactorySpec extends FunSpec {
  ...
  describe("----------------------------\n:  TypeAdapter Type Tests  :\n----------------------------") {
    ...
    it("Find with =:= (one type param)") {
      context.typeAdapterOf[Set[Any]].getClass.getName should be("co.blocke.scalajack.json.misc.TA6")
    }
    it("Find with <:< (one type param)") {
    ...
    it("Find with =:= (two type params)") {
      context.typeAdapterOf[ListMap[Any, Any]].getClass.getName should be("co.blocke.scalajack.json.misc.TA7")
    }
    it("Find with <:< (two type params)") {
    ...
    it("Register a simple/primitive TypeAdapter") {
      ...
      c2.typeAdapterOf[Boolean].getClass.getName should be("co.blocke.scalajack.json.misc.TAx")
    }
  }
}
