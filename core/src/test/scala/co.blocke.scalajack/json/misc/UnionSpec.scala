...
class UnionSpec extends AnyFunSpec with Matchers {
  ...
  describe("---------------------\n:  Union Tests  :\n---------------------") {
    it("Union2") {
      ...
      val js3 = """{"one":true}"""
      val msg =
        """Failed to read any kind of a Union value
          |{"one":true}
          |-------^""".stripMargin
      the[ScalaJackError] thrownBy sj.read[Multi2](js3) should have message msg
    }
    it("Union3") {
      ...
      val msg =
        """Failed to read any kind of a Union value
          |{"one":12.34}
          |-------^""".stripMargin
      the[ScalaJackError] thrownBy sj.read[Multi3](js9) should have message msg
    }
    it("Union4") {
