  ...
  describe("-----------------------------\n:  Custom Type Hints Tests  :\n-----------------------------") {
    describe("+++ Positive Tests +++") {
      it("Override default trait/polymorphic type hint") {
        ...
        assertResult("""{"which":"co.blocke.scalajack.json.test.custom.USAddress","street":"123 Main","city":"New York","state":"NY","postalCode":"39822"}""") { js }
        assertResult(inst) {
          sj.read[Address](js)
        }
      }
      it("Override type-specific trait/polymorphic type hint") {
        ...
        val js = sj.render(inst)
        assertResult("""{"_hint":"co.blocke.scalajack.json.test.custom.USDemographic","age":50,"address":{"addr_kind":"co.blocke.scalajack.json.test.custom.USAddress","street":"123 Main","city":"New York","state":"NY","postalCode":"39822"}}""") { js }
        assertResult(inst) {
          sj.read[Demographic](js)
        }
      }
      it("Use ClassNameHintModifier to modify trait/polymorphic type hint value") {
        val prependHintMod = ClassNameHintModifier((hint: String) => "co.blocke.scalajack.json.test.custom." + hint, (cname: String) => cname.split('.').last)
        val sj = ScalaJack().withHintModifiers((typeOf[Address], prependHintMod))
        val inst: Demographic = USDemographic(50, USAddress("123 Main", "New York", "NY", "39822"))
        val js = sj.render(inst)
        assertResult("""{"_hint":"co.blocke.scalajack.json.test.custom.USDemographic","age":50,"address":{"_hint":"USAddress","street":"123 Main","city":"New York","state":"NY","postalCode":"39822"}}""") { js }
        assertResult(inst) {
          sj.read[Demographic](js)
        }
      }
      it("Use StringMatchHintModifier to modify trait/polymorphic type hint value") {
        ...
        val inst: Demographic = USDemographic(50, USAddress("123 Main", "New York", "NY", "39822"))
        val js = sj.render(inst)
        assertResult("""{"_hint":"co.blocke.scalajack.json.test.custom.USDemographic","age":50,"address":{"_hint":"US","street":"123 Main","city":"New York","state":"NY","postalCode":"39822"}}""") { js }
        assertResult(inst) {
          sj.read[Demographic](js)
        }
      }
    ...
    describe("--- Negative Tests ---") {
      ...
      it("Hint value after modification doesn't resolve to known class name") {
        ...
        val msg = """[$.address]: Failed to apply type modifier to type member hint USAddress
                    |Demographic","age":50,"address":{"_hint":"USAddress","street":"123 Main","city":"New York","state":"
                    |--------------------------------------------------^""".stripMargin
      ...
      it("Unknown string given as type hint value (no cooresponding match to class in mapping)") {
        ...
        val msg = """[$.address]: Failed to apply type modifier to type member hint Bogus
                    |m.USDemographic","age":50,"address":{"_hint":"Bogus","street":"123 Main","city":"New York","state":"
                    |--------------------------------------------------^""".stripMargin
        the[co.blocke.scalajack.model.ReadInvalidError] thrownBy sj.read[Demographic](js) should have message msg
      }
