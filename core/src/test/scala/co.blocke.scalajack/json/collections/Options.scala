...
class Options() extends AnyFunSpec with Matchers {
  ...
  describe("------------------\n:  Option Tests  :\n------------------") {
    ...
    it("Option of Case Class") {
      ...
      val inst3: Map[Option[SomeClass], Int] =
    ...
    it("Option is None (value in Map)") {
      val inst: Map[Int, Option[String]] =
        Map(1 -> Some("one"), 2 -> None, 3 -> Some("three"))
      val js = sj.render(inst)
    ...
    it("Option is None (in Tuple)") {
      ...
      val js = sj.render(inst)
      assertResult(
        """[{"foo":1,"t":[true,"ok",2]},{"foo":5,"t":[false,null,3]}]"""
      ) { js }
      assertResult(inst) {
    ...
    it("Reading null into optional List item") {
      val js = """[1,null,2]"""
      assertResult(List(Some(1), null, Some(2))) {
        sj.read[List[Option[Int]]](js)
      }
    }
    it("Reading null into optional Map item") {
      val js = """{"1":"one","2":null,"3":"three"}"""
      assertResult(Map(1 -> Some("one"), 2 -> None, 3 -> Some("three"))) {
        sj.read[Map[Int, Option[String]]](js)
      }
    }
