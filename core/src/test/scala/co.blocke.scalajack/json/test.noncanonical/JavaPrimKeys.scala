    ...
    describe("+++ Positive Tests +++") {
      describe("Simple Primitives:") {
        ...
        it("With Number Key") {
          ...
          val instUpgraded = SampleJNumber(Map(
            new JLong("-128") -> new JLong("127"),
            new JLong("-32768") -> new JLong("32767"),
            new JLong("-2147483648") -> new JLong("2147483647"),
            new JLong("-9223372036854775808") -> new JLong("9223372036854755807"),
            new JLong("0") -> new JBigInteger("9923372036854755810"),
            new JDouble("3.4e-038") -> new JDouble("3.4e+038"),
            new JDouble("1.7e-308") -> new JDouble("1.7e+308"),
            new JBigDecimal("1.8e+308") -> new JDouble("0.0")))
          val js = sj.render(inst)
          ...
          assertResult(instUpgraded) { read } // upon deserialization "small" types get "upgraded" to their larger counterparts
        }
        it("With Short Key") {
    ...
    describe("--- Negative Tests ---") {
      describe("Simple Primitives:") {
        ...
        it("Bad Char Key") {
          val js = """{"m":{-3:"A","z":"Z"}}"""
          val msg = """DeserializationException(1 error):
                      |  [$.m.-3] Expected a char (JSON string of length 1), not -3 (reported by: co.blocke.scalajack.typeadapter.CharDeserializer)""".stripMargin
        ...
        it("Bad Number Key") {
          val js = """{"m":{"0xr":9923372036854755810,-2147483648:2147483647,-9223372036854775808:9223372036854755807,-128:127,3.4E-38:3.4E38,-32768:32767,1.8E+308:0.0,1.7E-308:1.7E308}}"""
          val msg = """DeserializationException(1 error):
                      |  [$.m.0xr] Expected a JSON number (reported by: co.blocke.scalajack.typeadapter.javaprimitives.BoxedNumberDeserializer)""".stripMargin
