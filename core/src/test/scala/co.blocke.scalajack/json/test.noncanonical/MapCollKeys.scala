...
class MapCollKeys() extends FunSpec with Matchers {
  ...
  describe("----------------------------\n:  Map Noncanonical Tests  :\n----------------------------") {
    ...
    it("Map of parameterized trait as key") {
      ...
      val inst = Map(Map(m1 -> m2) -> Map(m2 -> m1))
      val js = sj.render(inst)
      assertResult("""{{{{"_hint":"co.blocke.scalajack.json.test.noncanonical.AThing","a":"one","b":1}:{"_hint":"co.blocke.scalajack.json.test.noncanonical.AThing","a":"two","b":2}}:{{"_hint":"co.blocke.scalajack.json.test.noncanonical.AThing","a":"four","b":4}:{"_hint":"co.blocke.scalajack.json.test.noncanonical.AThing","a":"three","b":3}}}:{{{"_hint":"co.blocke.scalajack.json.test.noncanonical.AThing","a":"four","b":4}:{"_hint":"co.blocke.scalajack.json.test.noncanonical.AThing","a":"three","b":3}}:{{"_hint":"co.blocke.scalajack.json.test.noncanonical.AThing","a":"one","b":1}:{"_hint":"co.blocke.scalajack.json.test.noncanonical.AThing","a":"two","b":2}}}}""") { js }
      assertResult(true) {
