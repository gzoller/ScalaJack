  ...
  describe("-------------------------------------\n:  Inheritance Tests (Plain Class)  :\n-------------------------------------") {
    describe("Scala Plain") {
      it("Simple class inheritance must work (all fields present)") {
        val js = """{"uno":"foo","foobar":99,"dontForget":12,"three":false,"quatro":12.34,"foo":25,"extra":"bar"}"""
        ...
        sj.render(simple) should be("""{"extra":"bar","uno":"foo","foo":25,"dontForget":12,"quatro":12.34,"three":false,"foobar":99}""")
      }
      it("MapName, and Ignore annotations must be inherited properly") {
        val adapter = sj.context.typeAdapterOf[InheritSimpleChild].asInstanceOf[typeadapter.classes.PlainClassTypeAdapter[_]]
        adapter.dbKeys.map(f => (f.name, f.dbKeyIndex)) should be(List(("uno", Some(0)), ("foobar", Some(1)), ("quatro", Some(2)), ("foo", Some(99))))
      ...
      it("With type member (as part of a trait)") {
        ...
        inst.rose = flower
        val js = sj.render(inst)
        js should be("""{"flower":"co.blocke.scalajack.json.primitives.plain.Flower","rose":{"thing":5,"other":6}}""")
        val inst2 = sj.read[WrapTrait[TraitBase]](js)
        inst2.rose should be(flower)
      }
    }
    ...
    describe("Java Plain") {
      it("Simple class inheritance must work (all fields present) including MapName and Ignore") {
