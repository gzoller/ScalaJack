...
class ClassParams extends AnyFunSpec with GivenWhenThen with BeforeAndAfterAll {
  ...
  describe("---------------------------------\n:  Class Paramterization Tests  :\n---------------------------------") {
    describe("Basic Parameterized Case Class") {
      ...
      it("Non-parameter case class as a parameter - Foo[A](x:A) where A -> Bar (case clas)") {
    ...
    describe("Advanced Parameterized Case Class") {
      ...
      it("Parameterized case class with parameterized another member - Foo[A](x:Bar[A], y:A)") {
      ...
      it("Class with two parameters, one given one not - Foo[A](x:List[Bar[A, Boolean]])") {
        val inst = Foo4(List(Bar3(Map(4 -> "yes", 5 -> "no"), true), Bar3(Map(8 -> "yellow", 9 -> "red"), false)), Map(1 -> "wow", 2 -> "yup"))
        val js = sj.render(inst)
        assertResult("""{"x":[{"id":{"4":"yes","5":"no"},"isIt":true},{"id":{"8":"yellow","9":"red"},"isIt":false}],"b":{"1":"wow","2":"yup"}}""") { js }
        assertResult(inst) {
    ...
    describe("Very Advanced Parameterized Case Class") {
      it("Multiple parameters, in order - Foo[A,B](x:Bar[A,B], y:A)") {
        val inst = Foo5(List(Bar3(Map(4 -> "yes", 5 -> "no"), true), Bar3(Map(8 -> "yellow", 9 -> "red"), false)), Map(1 -> "wow", 2 -> "yup"))
        val js = sj.render(inst)
        assertResult("""{"x":[{"id":{"4":"yes","5":"no"},"isIt":true},{"id":{"8":"yellow","9":"red"},"isIt":false}],"b":{"1":"wow","2":"yup"}}""") { js }
        assertResult(inst) {
      ...
      it("Multiple parameters, out of order - Foo[A,B,C,D](x:Bar[C,D,A], y:B)") {
        ...
        val js = sj.render(inst)
        assertResult("""{"x":{"id":5,"thing1":2.5,"thing2":"H"},"y":"wow"}""") { js }
        assertResult(inst) {
      ...
      it("Nested multiple parameters, out of order - Foo[A,B,C,D](x:Bar[C,Blah[D,A]], y:B)") {
        ...
        val js = sj.render(inst)
        assertResult("""{"x":{"id":5,"blah":{"t":2.5,"u":"H"}},"y":"wow"}""") { js }
        assertResult(inst) {
