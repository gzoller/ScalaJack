      ...
      describe("+++ Positive Tests +++") {
        describe("Simple DelimSpec:") {
          ...
          it("With Number Key") {
            ...
            parseJValue(js) should matchJson(
              parseJValue(
                """{"m":{"0":9923372036854755810,"-2147483648":2147483647,"-9223372036854775808":9223372036854755807,"-128":127,"3.4E-38":3.4E38,"-32768":32767,"1.8E+308":0.0,"1.7E-308":1.7E308}}"""
              )
            )
        ...
        describe("Time DelimSpec:") {
          ...
          it("With Instant Key") {
            ...
            val js = sj.render(inst)
            assertResult(
              """{"m":{"1970-01-01T00:00:00Z":"+1000000000-12-31T23:59:59.999999999Z","-1000000000-01-01T00:00:00Z":"2007-12-03T10:15:30Z"}}"""
            ) { js }
            assertResult(inst) {
          ...
          it("With LocalDateTime Key") {
            ...
            val js = sj.render(inst)
            assertResult(
              """{"m":{"+999999999-12-31T23:59:59.999999999":"-999999999-01-01T00:00:00","2007-12-03T10:15:30":null}}"""
            ) { js }
            assertResult(inst) {
          ...
          it("With LocalDate Key") {
            ...
            val js = sj.render(inst)
            assertResult(
              """{"m":{"+999999999-12-31":"-999999999-01-01","2007-12-03":null}}"""
            ) { js }
            assertResult(inst) {
          ...
          it("With LocalTime Key") {
            ...
            val js = sj.render(inst)
            assertResult(
              """{"m":{"23:59:59.999999999":"00:00:00","00:00:00":"12:00:00","10:15:30":null}}"""
            ) { js }
            assertResult(inst) {
          ...
          it("With OffsetDateTime Key") {
            ...
            val js = sj.render(inst)
            assertResult(
              """{"m":{"+999999999-12-31T23:59:59.999999999-18:00":"-999999999-01-01T00:00:00+18:00","2007-12-03T10:15:30+01:00":null}}"""
            ) { js }
            assertResult(inst) {
          ...
          it("With OffsetTime Key") {
            ...
            val js = sj.render(inst)
            assertResult(
              """{"m":{"23:59:59.999999999-18:00":"00:00:00+18:00","10:15:30+01:00":null}}"""
            ) { js }
            assertResult(inst) {
        ...
        describe("Simple DelimSpec:") {
          it("Bad BigDecimal Key") {
            ...
            the[ScalaJackError] thrownBy sj
              .read[SampleJBigDecimal](js) should have message msg
          }
          it("Bad BigInt Key") {
            ...
            val msg = """Expected a Number here
                      ...
                      |^""".stripMargin
            the[ScalaJackError] thrownBy sj
              .read[SampleJBigInteger](js) should have message msg
          }
          it("Bad Boolean Key") {
            ...
            val msg = """Expected a Boolean here
                      |123
                      |^""".stripMargin
            the[ScalaJackError] thrownBy sj.read[SampleJBoolean](js) should have message msg
          }
          it("Bad Byte Key") {
            val js = """{"m":{"16":2,"4x8":9}}"""
            val msg = """Expected a Number here
                      |4x8
                      |-^""".stripMargin
            the[ScalaJackError] thrownBy sj.read[SampleJByte](js) should have message msg
          }
          it("Bad Char Key") { // NOTE: This comprehensively tests for any null keyed Map
            ...
            val msg = """Map keys cannot be null
                      |{"m":{null:"A","z":"Z"}}
                      |----------^""".stripMargin
            the[ScalaJackError] thrownBy sj.read[SampleJChar](js) should have message msg
          }
          it("Bad Double Key") {
            ...
            val msg = """Expected a Number here
                      |true
                      |^""".stripMargin
            the[ScalaJackError] thrownBy sj.read[SampleJDouble](js) should have message msg
          }
          it("Bad Float Key") {
            val js = """{"m":{"12.34":56.78,"90.12.3":34.56}}"""
            the[java.lang.NumberFormatException] thrownBy sj.read[SampleJFloat](
              js
            ) should have message "multiple points"
          }
          it("Bad Int Key") {
            val js = """{"m":{"12.0":56,"90":34}}"""
            the[java.lang.NumberFormatException] thrownBy sj.read[SampleJInteger](
              js
            ) should have message "For input string: \"12.0\""
          }
          it("Bad Long Key") {
            ...
            val msg = """Expected a Number here
                      |hey
                      |^""".stripMargin
            the[ScalaJackError] thrownBy sj.read[SampleJLong](js) should have message msg
          }
          it("Bad Number Key") {
            val js =
              """{"m":{"flume":9923372036854755810,"-2147483648":2147483647,"-9223372036854775808":9223372036854755807,"-128":127,"3.4E-38":3.4E38,"-32768":32767,"1.8E+308":0.0,"1.7E-308":1.7E308}}"""
            val msg = """Expected a Number here
                      |flume
                      |^""".stripMargin
            the[ScalaJackError] thrownBy sj.read[SampleJNumber](js) should have message msg
          }
        }
        describe("Time DelimSpec:") {
          it("Bad Duration Key") {
            ...
            val msg = """Failed to parse Duration from input 'PT0SXXX'
                      |{"m":{"PT0SXXX":"PT51H4M"}}
                      |--------------^""".stripMargin
            the[ScalaJackError] thrownBy sj.read[SampleDuration](js) should have message msg
          }
          it("Bad Instant Key") {
            val js =
            ...
            val msg =
              """Failed to parse Instant from input 'bogus'
                      ...
                      |----------------------------------------------------^""".stripMargin
            the[ScalaJackError] thrownBy sj.read[SampleInstant](js) should have message msg
          }
          it("Bad LocalDateTime Key") {
            val js =
              """{"m":{"+999999999-12-31T23:59:59.999999999":"-999999999-01-01T00:00:00","bogus":null}}"""
            val msg =
              """Failed to parse LocalDateTime from input 'bogus'
                      |...:59.999999999":"-999999999-01-01T00:00:00","bogus":null}}
                      |----------------------------------------------------^""".stripMargin
            the[ScalaJackError] thrownBy sj
              .read[SampleLocalDateTime](js) should have message msg
          }
          it("Bad LocalDate Key") {
            ...
            val msg = """Failed to parse LocalDate from input 'bogus'
                      ...
                      |------------^""".stripMargin
            the[ScalaJackError] thrownBy sj
              .read[SampleLocalDate](js) should have message msg
          }
          it("Bad LocalTime Key") {
            ...
            val msg =
              """Failed to parse LocalTime from input 'nada'
                      ...
                      |-------------------------------------------^""".stripMargin
            the[ScalaJackError] thrownBy sj
              .read[SampleLocalTime](js) should have message msg
          }
          it("Bad OffsetDateTime Key") {
            val js =
              """{"m":{"false":"-999999999-01-01T00:00:00+18:00","2007-12-03T10:15:30+01:00":null}}"""
            val msg =
              """Failed to parse OffsetDateTime from input 'false'
                      |{"m":{"false":"-999999999-01-01T00:00:00+18:00","2007-12-03T10:15:30+01:00":n...
                      |------------^""".stripMargin
            the[ScalaJackError] thrownBy sj
              .read[SampleOffsetDateTime](js) should have message msg
          }
          it("Bad OffsetTime Key") {
            ...
            val msg =
              """Failed to parse OffsetTime from input '2007-12-03T10:15:30+01:00[Europe/Bogus]'
                      ...
                      |-----------------------------------------------^""".stripMargin
            the[ScalaJackError] thrownBy sj
              .read[SampleOffsetTime](js) should have message msg
          }
          it("Bad Period Key") {
            ...
            val msg = """Failed to parse Period from input 'P0D???'
                      |{"m":{"P0D???":"P1Y2M3D"}}
                      |-------------^""".stripMargin
            the[ScalaJackError] thrownBy sj.read[SamplePeriod](js) should have message msg
          }
          it("Bad ZonedDateTime Key") {
            ...
            val msg =
              """Failed to parse ZonedDateTime from input 'FRED23:59:59.999999999-18:00'
                      ...
                      |-----------------------------------^""".stripMargin
            the[ScalaJackError] thrownBy sj
              .read[SampleZonedDateTime](js) should have message msg
          }
        }
      }
    }
