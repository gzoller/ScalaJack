...
class Plain extends AnyFunSpec with Matchers {
    ...
    "--------------------------------\n:  DynamoDB Plain Class Tests  :\n--------------------------------"
      ...
      describe("Items:") {
        it("All val constructor") {
          ...
          parseJValue(
            """{"name":"Greg","age":50,"likes":["Woodworking","Diet Coke"],"stuff":{"wow":1.23,"bing":"boom"},"foo":true}"""
          ) should matchJson(parseJValue(item.toJSON))
          assertResult(true) {
        ...
        it("Zero-arg constructor with var members") {
          ...
          val item = sj.render(inst)
          parseJValue(
            """{"name":"Greg","age":50,"likes":["Woodworking","Diet Coke"],"stuff":{"wow":1.23,"bing":"boom"}}"""
          ) should matchJson(parseJValue(item.toJSON))
          assertResult(true) {
