...
class Basics extends AnyFunSpec {
    ...
    "--------------------------------\n:  DynamoDB Basic Value Tests  :\n--------------------------------"
      ...
      describe("Extended Serialization:") {
        it("Custom type adapter") {
          ...
          val item = sj.render(inst)
          assertResult("""{ Item: {name=Bartholomew, phone=555-555-5555} }""") {
            item.toString
          }
          assertResult(inst) {
        ...
        it("With Hints") {
          val sj =
            ScalaJack(DynamoFlavor()).withHints((typeOf[Address] -> "addr_kind"))
        ...
        it("With Hint Modifiers") {
          ...
          val sj = ScalaJack(DynamoFlavor())
            .withHintModifiers((typeOf[Address], prependHintMod))
          val inst: Address = USAddress("123 Main", "New York", "NY", "39822")
          val item = sj.render(inst)
          assertResult(
