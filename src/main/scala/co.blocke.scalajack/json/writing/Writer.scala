...
object Writer:
  ...
  private def makeWriteFn[U: Type](
    ...
    Apply(
      Ref(
        ctx.writeMethodSyms.getOrElse(
          methodKey, {
            val sym = Symbol.newMethod(
              Symbol.spliceOwner,
              "w" + ctx.writeMethodSyms.size, // 'w' is for Writer!
              MethodType(List("in", "out"))(_ => List(TypeRepr.of[U], TypeRepr.of[JsonOutput]), _ => TypeRepr.of[Unit])
            )
            ctx.writeMethodSyms.update(methodKey, sym)
      ...
      case '[b] =>
        r match
          ...
          case t: SelfRefRef[?] =>
            ...
            // For now burp out a null. Go back later and resolve SelfRef syms before making codec
