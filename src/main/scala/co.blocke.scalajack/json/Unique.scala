...
case class Unique(fingerprints: Map[String, List[ScalaClassRef[?]]], optionalFields: Set[String]):
  ...
  lazy val simpleUniqueHash: Map[String, String] =
    fingerprints.collect { case (hash, List(singleClass: ScalaClassRef[?])) =>
      hash -> singleClass.name
    }.toMap
...
object Unique:
  ...
  private inline def isOptional(fr: FieldInfoRef): Boolean =
    fr.fieldRef match {
