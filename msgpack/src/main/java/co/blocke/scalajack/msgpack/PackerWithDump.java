...
//
package co.blocke.scalajack.msgpack;
...
public class PackerWithDump extends AbstractPacker {
    protected final Output out;

    private PackerStack stack = new PackerStack();

    public PackerWithDump(MessagePack msgpack, OutputStream stream) {
        this(msgpack, new StreamOutput(stream));
    }

    protected PackerWithDump(MessagePack msgpack, Output out) {
        super(msgpack);
        this.out = out;
    }

    // Just dump a pre-written byte array with no prefixing -- the prefix is already
    // cooked in the given bytes.  (Used for Polymorphic handling in ScalaJack)
    public void dumpBytes(byte[] packaged) throws IOException {
